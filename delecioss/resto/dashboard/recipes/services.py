...
def get_recipe_list():
    sql = """
    select rec.id, rec.name, rec.steps, rec.date, rec.prep, rec.cook, rec.yields,
     rec.img, ctg.content as ctg_name, rec.rate
...
def delete(id=None, cat=None):
    if cat:
        sql = "delete from recipe_recipe where cat_id=%s"
        with closing(connection.cursor()) as cursor:
